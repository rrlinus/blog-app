...
app = Flask(__name__)
app.secret_key = 'try to use as strong as possible'
...
def edit(sno):
    if 'user' in session and session['user'] == params['admin_user']:
        if request.method=='POST':
            ...
            date=datetime.now()
            if sno=='0':
                post=Posts(title=box_title,tagline=tagline,content=content,slug=slug,img_file=img_file,date=date)
                print(post.tagline,post.title)
                db.session.add(post)
                db.session.commit()
            else:
...
def contact():
    ...
    if request.method=="POST":
        ...
        if not name or not email or not phone:
            flash("sorry,please fill all the manadatory details!!!")
            p="alert alert-danger"
            return render_template('contact.html', params=params,p=p)
